...
mkdir -p 'data/Shapes/ml/experiment_9/logs/' 'data/Shapes/ml/experiment_9/snapshots/' 'data/Shapes/ml/experiment_9/aggregated' 'data/Shapes/ml/experiment_9/transfer'
...
do
	for fold in $folds
	do
		$cmd $ann_script data/Shapes/ml/dataset/Shapes.pickle data/Shapes/ml/dataset/Additional.pickle data/Shapes/ml/dataset/Berlin.pickle data/Shapes/ml/dataset/Sketchy.pickle data/Shapes/ml/dataset/targets.pickle 'mean_'"$dim" data/Shapes/images/ data/Shapes/mds/similarities/aggregator/mean/aggregated_ratings.pickle data/Shapes/ml/experiment_9/multi-task.csv $ann_config -s 42 -f $fold $walltime --initial_stride 3 --image_size $image_size --noise_only_train --patience $patience --epochs $epochs
	done
done
...
python -m code.ml.ann.average_folds data/Shapes/ml/experiment_9/multi-task.csv data/Shapes/ml/experiment_9/aggregated/multi-task.csv
for dim in $dims
do
