...
do
	...
	do
		echo "            $baseline"	
		python code/regression/regression.py data/NOUN/dataset/targets.pickle "$target"'_4' data/NOUN/dataset/features_inception.pickle 'data/NOUN/ML_results/experiment_2/'"$target"'/baselines.csv' -s 42 $baseline
	done
	...
	do
		...
		do
			echo "                $regressor"
			python code/regression/regression.py data/NOUN/dataset/targets.pickle "$target"'_4' 'data/NOUN/dataset/features_'"$feature_set"'.pickle' 'data/NOUN/ML_results/experiment_2/'"$target"'/'"$feature_set"'.csv' -s 42 --shuffled $regressor
		done

		for lasso in $lassos
		do
			echo "            lasso $lasso"
			python code/regression/regression.py data/NOUN/dataset/targets.pickle "$target"'_4' 'data/NOUN/dataset/features_'"$feature_set"'.pickle' 'data/NOUN/ML_results/experiment_2/'"$target"'/'"$feature_set"'.csv' -s 42 --lasso $lasso
		done
