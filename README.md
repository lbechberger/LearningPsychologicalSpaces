...
#### 2.1.12 Comparing Visual and Conceptual Similarity Ratings

You can use the script `analyze_visual_conceptual.r` to compare the visual and the conceptual similarity ratings and to analyze whether they have a statistically significant difference:
```
Rscript code/mds/preprocessing/analyze_visual_conceptual.r -c path/to/conceptual.csv -v path/to/visual.csv
```
Here, `conceptual.csv` and `visual.csv` are the csv files generated by `create_similarity_csv.py`.
