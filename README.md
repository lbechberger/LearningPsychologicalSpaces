...
The script `analyze_concept_size.py` evaluates category size by computing the average distance to the category prototype for all categories. *This is only applicable to the Shapes data set, as there are no categories in NOUN.* The script can be invoked as follows (where `vectors.pickle` is the output of `normalize_vectors.py` and `n_dims` is the dimensionality of the space to consider), and `output_file.csv` serves as storage location for the analysis results):
```
