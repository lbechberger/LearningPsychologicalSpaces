...
#### 2.1.3 Parsing Shapes Dimension Data

The script `preprocess_dimension.py` reads in the dimension ratings (both binary and continuous) from two given CSV files and stores them in a single pickle file for further usage. It can be invoked as follows:
```
python -m code.mds.preprocessing.preprocess_dimension path/to/binary_ratings.csv path/to/continuous_ratings.csv path/to/output.pickle
```
...
In order to analyze the dimension ratings, we can use the script `analyze_dimension.py` which can be called as follows:
```
python -m code.mds.preprocessing.analyze_dimension path/to/input.pickle path/to/analysis_folder path/to/classification.pickle path/to/regression.pickle
```

Here, `input.pickle` is the file generated by `preprocess_dimension.py`. The script analyzes the binary and continuous ratings for the given dimension and stores a scatter plot in the given `analysis_folder`. Information about the analysis results are printed to the console. 
...
- `'continuous_mean'`: Classification information based on the continuous rating responses, aggregated with the arithmetic mean.
- `'continuous_median'`: Classification information based on the continuous rating responses, aggregated with the median.
...
- `'continuous_mean'`: Regression information based on the continuous rating responses, aggregated with the arithmetic mean.
- `'continuous_median'`: Regression information based on the continuous rating responses, aggregated with the median.
...
- `-r` or `--response_times`: If this flag is set, the response times from the binary classification are also converted into classification and regression information. This creates additional entries in the pickle files, namely `'rt_mean'` and `'rt_median'`.

#### 2.1.8 Correlations between Interpretable Dimensions

The script `compare_dimensions.py` compares the scales of two different interpretable dimensions to each other, based on each of the scale variants. It can be invoked as follows:
```
python -m code.mds.preprocessing.compare_dimensions path/to/first.pickle path/to/second.pickle path/to/output_folder
```
