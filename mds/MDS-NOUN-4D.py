...
# Multi-dimensional Scaling (4 dimensions)
mds = manifold.MDS(n_components=4, dissimilarity="precomputed", random_state=None)
results = mds.fit(data_array)
coords = results.embedding_
print(coords)

# write the coordinates in structured format into a csv file
with open("output/4D-vectors.csv", 'w') as f:
    for i in range(64):
...
# plot other two dimensions (x)
plt.subplots_adjust(bottom=0.1)
plt.scatter(
	coords[:, 2], coords[:, 3], marker='x'
)
for label, x, y in zip(image_array, coords[:, 2], coords[:, 3]):
	plt.annotate(
		label,
		xy=(x, y), xytext=(-20, 20),
		textcoords='offset points', ha='right', va='bottom',
